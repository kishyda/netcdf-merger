...
#[cfg(all(feature = "has-mmap", feature = "4.6.2"))]
#[repr(C)]
struct NcMemio {
    size: usize,
...
impl RawFile {
    ...
    #[cfg(all(feature = "has-mmap", feature = "4.6.2"))]
    fn close_to_bytes(self) -> error::Result<Vec<u8>> {
        let ncid = self.ncid;
        std::mem::forget(self);

        let mut memio = MaybeUninit::<NcMemio>::zeroed();
        checked_with_lock(|| unsafe { nc_close_memio(ncid, memio.as_mut_ptr().cast()) })?;

        let memio = unsafe { memio.assume_init() };
        let ptr = memio.memory.cast::<u8>();
        let len = memio.size;

        if ptr.is_null() {
            return Err("nc_close_memio returned null memory".into());
        }
...
impl RawFile {
    ...
    #[cfg(all(feature = "has-mmap", feature = "4.6.2"))]
    pub(crate) fn create_inmemory(initial_size: usize) -> error::Result<FileMut> {
...
impl FileMut {
    ...
    #[cfg(all(feature = "has-mmap", feature = "4.6.2"))]
    /// Close an in-memory file and return the serialized NetCDF bytes.
    ///
    /// This is intended for files created with [`crate::create_mem`].
    pub fn close_to_bytes(self) -> error::Result<Vec<u8>> {
        let Self(File(file)) = self;
        file.close_to_bytes()
    }
