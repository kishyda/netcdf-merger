...
/// Create a new in-memory `netCDF` file that can later be serialized with
/// [`FileMut::close_to_bytes`].
pub fn create_mem(initial_size: usize) -> error::Result<FileMut> {
    RawFile::create_inmemory(initial_size)
}
